...
class PathFinder:
    ...
    def to_coordinates(self, grid_x, grid_y):
        x = grid_x * self.global_width / self.grid_width
        y = grid_y * self.global_height / self.grid_height
        return x, y
        # return grid_x, grid_y

    def find_path(
        self, start, goal, obstacles, distance_metric="manhattan", debug: bool =False
    ):
        ...
        if debug is True:
            ...
            return ([self.to_coordinates(x, y) for x, y in path_grid], ddict)
        else:
            path_grid = a_star(start_grid, goal_grid, obstacles_grid)
            return [self.to_coordinates(x, y) for x, y in path_grid]
