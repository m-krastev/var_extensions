...
class LineDetectionNode(Node):
    def __init__(self, use_canny=False, show_battery=False):
        ...
        self.battery_reader = self.create_subscription(
            BatteryState, BATTERY_STATUS_TOPIC, self.battery_callback, 10
        )
    ...
    def detect_line_ours(self, image):
        ...
        for i, contour in enumerate(contours[:top_n]):
            ...
            b = vx
            c = vy * x0 - vx * y0
            errors = np.abs(a * points[:, 0] + b * points[:, 1] + c) / np.sqrt(
                a**2 + b**2
            )
    ...
    def detect_line_canny(self, image):
        ...
        if lines is not None:
            ...
            for line in lines:
                ...
                if (
                    line_brightness > max_brightness
                    and abs(line_center_x - image_center_x) < 50
                ):
...
def main(args=None):
    ...
    parser.add_argument(
        "--use_canny", action="store_true", help="Use Canny edge detection"
    )
    parser.add_argument(
        "--show_battery", action="store_true", help="Show battery status on the LCD"
    )
